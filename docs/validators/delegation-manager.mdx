...
_Step 1: choose the environment_
...
_Step 2: create a directory with an unique name_
Must be lowercase, alphanumeric, with no spaces. For example, `my-new-identity`

_Step 3: add a `info.json` file_
...
In order to validate the ownership of a staking provider or validator nodes, one will need to sign a message by using the owner wallet and then submit a comment on that PR.
