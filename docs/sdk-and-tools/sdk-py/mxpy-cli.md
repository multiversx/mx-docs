...
If you are using an older version of `mxpy` you can simply upgrade to a newer version by typing the following commands in a terminal:
...
We see that `mxpy` returns our value as a base64 string, as a hex number and as an integer. Indeed, we see the expected value.
