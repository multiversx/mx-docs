...
```
:::warning Decimal accuracy in Contracts
When working with ESDT tokens in smart contracts or transactions, always account for the token's decimal places.

**Example with 18 decimals:**
* To send 10 tokens: use `10000000000000000000` (10 × 10^18)
* NOT `10`
```rust
// Wrong - sends 0.000000000000000010 tokens
let amount = BigUint::from(10u64);

// Correct - sends 10 tokens with 18 decimals
let amount = BigUint::from(10u64) * BigUint::from(10u64).pow(18);
```

Forgetting decimal places is a common cause of "amount mismatch" errors in contracts.
:::
...
:::caution Token Identifier Format
ESDT token identifiers always include a random 6-character suffix after a hyphen (e.g., `TOKEN-abc123`). This suffix is automatically generated by the protocol when the token is issued and cannot be chosen or predicted.

**Common mistake:** Using just the ticker (`TOKEN`) instead of the full identifier (`TOKEN-abc123`) will cause "wrong token sent" errors in smart contracts that validate token IDs.

Always use the complete identifier including the suffix in:
* Smart contract deployment arguments
