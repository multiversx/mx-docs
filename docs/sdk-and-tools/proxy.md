...
Overview of the MultiversX (previously Elrond) Proxy
...
1. The **MultiversX (previously Elrond) Network** - consisting of Nodes grouped within Shards. Some of these Nodes are **Observers**.
2. One or more instances of the **MultiversX (previously Elrond) Proxy** - including the official one - connect to Observer Nodes in order to forward incoming user Transactions to the Network and to query state within the Blockchain.
3. The **client applications** connect to the Network through the MultiversX (previously Elrond) Proxy. It is also possible for a blockchain-powered application to talk directly to an Observer or even to a Validator.

## **Official MultiversX (previously Elrond) Proxy**

The official instance of the MultiversX (previously Elrond) Proxy is located at [https://gateway.multiversx.com](https://gateway.multiversx.com/).
...
- `external.toml` - this file holds configuration necessary to Proxy components that interact with external systems. An example of such an external system is **Elasticsearch** - currently, MultiversX (previously Elrond) Proxy requires an Elasticsearch instance to implement some of its functionality.
