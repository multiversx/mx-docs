...
description: A contract exercising six storage mappers side by side, manually verified when written but not continuously built in CI, with a source-grounded decision table.
...
Index 0 is invalid and panics. In the authoring-time manual test, the recipe
pushed two items and read back index 1 as the first one pushed.
:::
