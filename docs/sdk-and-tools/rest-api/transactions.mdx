...
| guardian         | <span class="text-danger">OPTIONAL</span> | `string` | The Address (bech32) of the Guardian.                                            |
...
| guardianSignature| <span class="text-danger">REQUIRED</span> | `string` | The Guardian's Signature (hex-encoded) of the Transaction.                       |
...
Here's an example of a **guarded transaction** request:
...
```
:::note
Both the guardian and sender have to sign:
```json
{
    "nonce": 2,
    "value": "0",
...
```
Notice the fact that neither the signature of the sender nor the signature of the guardian need to be completed for the signature action.
:::
