...
Let's build your first decentralized App on the MultiversX (previously Elrond) Blockchain
...
Maybe you noticed the default amount for a deposit is **1 xEGLD** and not 1 EGLD which is MultiversX (previously Elrond) official token, this is because, for testing purposes we use MultiversX (previously Elrond) Devnet, which is a testing environment identical to the Mainnet, the official MultiversX (previously Elrond) Blockchain. Here, the currency is **xEGLD**, it's just a test token, it's worth nothing.

## **MultiversX (previously Elrond) dApp architecture**
...
The MultiversX (previously Elrond) _devnet_ is a public test network maintained by our community where any developer can test their smart contracts and dApps in a real world environment.
...
We'll also need `erdpy`, the MultiversX (previously Elrond) command line tool, which is helpful for signing transactions, deploying smart contracts, managing wallets, accounts and validators. We'll use it to deploy our smart contract.
Erdpy can be installed using the MultiversX (previously Elrond) documentation page [https://docs.multiversx.com/sdk-and-tools/erdpy/installing-erdpy](https://docs.multiversx.com/sdk-and-tools/erdpy/installing-erdpy)
...
Let's head over to the MultiversX (previously Elrond) wallet, click **"Create new wallet"**, write down the security phrase (24 words) that can help us retrieve the wallet, and the password for the JSON keystore (that we will save in the `~/ping-pong/wallet folder`). We should be able to see our new MultiversX (previously Elrond) wallet owner address which is, in this case, _erd1......._
...
Deploy the smart contract on MultiversX (previously Elrond) Devnet
...
Devnet Explorer will be your best friend in developing dApps on the MultiversX (previously Elrond) Blockchain, as you'll first deploy and test your dApps on Devnet.
...
INFO:utils:View this contract address in the MultiversX (previously Elrond) Devnet Explorer: https://devnet-explorer.elrond.com/accounts/erd1qqqqqqqqqqqqqpgq0hmfvuygs34cgqsvgg6fpq9c5mffh4y04cysagr6cn
...
INFO:utils:View this transaction in the MultiversX (previously Elrond) Devnet Explorer: https://devnet-explorer.elrond.com/transactions/ee84f3e833d439e159c9619fd76e26d2afcdad62c197d87e4940072f18558153
...
We also have two other functions defined in the smart contract: `get_time_to_pong` and `did_user_ping`, these view functions are invoked using **MultiversX (previously Elrond) API** (*https://devnet-api.multiversx.com/vm-values/query*).
...
The purpose of this guide is to provide a starting point for you to discover the MultiversX (previously Elrond) technology capabilities and devkit. Keep reading the next docs to dive in deeper.
...
Break down this guide and learn more about how to extend the smart contract, the wallet and the MultiversX (previously Elrond) tools. [https://docs.multiversx.com](/)
