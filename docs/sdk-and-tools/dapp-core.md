...
Library used to build React dApps on MultiversX (previously Elrond) Network.
...
dapp-core is a library that holds core functional logic that can be used to create a dApp on MultiversX (previously Elrond) Network.
...
dapp-core aims to abstract and simplify the process of interacting with users' wallets and with the MultiversX (previously Elrond) Network, allowing developers to easily get started with a new application or integrate dapp-core into an existing application.
