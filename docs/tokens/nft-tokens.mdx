...
It is worth noting that Meta ESDTs information displayed by the Explorer and returned by the API and the ElasticSearch are similar to the one of SFTs.
...
- [SDK and Tools](https://docs.multiversx.com/sdk-and-tools/overview) - SDK and Tools for MultiversX.
...
- If you are using IPFS, we strongly recommend not using the gateway URL of the provider (Pinata Cloud, Infura, Cloudflare, etc.) but rather the default IPFS one (https://ipfs.io/ipfs/ [ipfsCID]). This ensures that the media remains available even if the provider goes offline, and other gateways can be used in the future.
