...
MultiversX (previously Elrond) network natively supports the issuance of custom tokens, without the need for contracts such as ERC20, but addressing the same use-cases.
You can create and issue an ESDT token from [MultiversX (previously Elrond) web wallet](https://wallet.elrond.com/) in a few steps. Let's go over these steps.

## **Prerequisites**

- A wallet on MultiversX (previously Elrond) Network.
- 0.05 EGLD issuance fee
- fees for transactions

## **Creating a fungible token from Web Wallet**

To get started, open up the [MultiversX (previously Elrond) web wallet](https://wallet.elrond.com/). You can create a new wallet if you do not have one or import your existing wallet. Here is a [guide](https://docs.multiversx.com/wallet/web-wallet/) to help you navigate.
...
You can transfer an amount of a token to another account. To get started, open up the [MultiversX (previously Elrond) web wallet](https://wallet.elrond.com/).
