...
As stated above, if the user wants 4091 tokens with the 6 decimals, then it has to issue $4091 * 10^6$ tokens so a total of `4091000000`. 
```
IssuanceTransaction {
...
Once this transaction is processed by the Metachain, Alice becomes the designated **manager of AliceTokens**, and is granted a balance of 4091000000 AliceTokens with 6 decimals (resulting in 4091 indivisible tokens). She can increase the total supply of tokens at a later time if needed. For more operations available to ESDT token managers, see [Token management](/developers/esdt-tokens#token-management).
