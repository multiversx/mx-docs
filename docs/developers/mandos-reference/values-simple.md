...
The Mandos value format is closely related to the [MultiversX (previously Elrond) serialization format](/developers/developer-reference/elrond-serialization-format). This is not by accident, Mandos is designed to make it easy to interact MultiversX (previously Elrond) contracts and their data.
...
On MultiversX (previously Elrond), smart contract addresses have a different format than user address - they start with 8 bytes of zero.
