...
The web wallet webhooks are links that point the user to the wallet to either login or populate a send transaction form with the provided arguments. Once the action is performed the user is redirected to the provided callback URL along with a success or error status.
