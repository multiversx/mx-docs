...
:::important
If an account has an ActiveGuardian and is Guarded, the codeMetadata of the account should be Guarded.
:::
...
:::caution
For the above response example, the account has already set the ActiveGuardian (through a SetGuardian transaction), guarded the account (with a GuardAccount transaction) and also set the pendingGuardian (with a unGuarded SetGuardian transaction). We chose this situation so that the entire blockInfo and guardianData is displayed.
:::
